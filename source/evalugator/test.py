...
def test_part_six():
    ...
    # call the function under test
    part_six_output = assessment.measure_and_print_execution_time_filter()
    try:
        # check: there are two items in the tuple
        assert len(part_six_output) == 2
        # check: confirm that there are two labels inside of the output
        assert "Result:" in part_six_output[0]
        assert "Time:" in part_six_output[1]
        # note: test case does not assert that the output is correct
        # because of the fact that the timing will vary based on
        # the speed of the computer that is running the program
        return True
    except AssertionError:
        return False
